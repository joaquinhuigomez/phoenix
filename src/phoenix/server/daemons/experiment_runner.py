...
from phoenix.server.api.helpers.playground_spans import get_db_experiment_run
...
class TaskJob(Job):
    ...
    def __init__(
        ...
        *,
        ...
        self.span_cost_calculator = span_cost_calculator
        ...
        if self._client is None:
            ...
            with anyio.fail_after(10, shield=True):
                async with self.db() as session:
                    self._client = await get_playground_client(
                        model_provider=self.prompt_version.model_provider,
                        model_name=self.prompt_version.model_name,
                        custom_provider_id=self.prompt_version.custom_provider_id,
                        session=session,
    ...
    async def execute(self) -> None:
        ...
        try:
            with anyio.fail_after(self.timeout):
                ...
                tools_for_client: PromptTools | None = None
                if self.tools:
                    tools_for_client = PromptTools.model_validate(
                        {"type": "tools", "tools": self.tools}
                    )
                ...
                tracer = Tracer(span_cost_calculator=self.span_cost_calculator)
                async for chunk in client.chat_completion_create(
                    messages=self.messages,
                    tools=tools_for_client,
                    response_format=self.prompt_version.response_format,
                ...
                db_traces = tracer.get_db_traces(project_id=self.project_id)
                db_trace = db_traces[0] if db_traces else None
                if db_trace is None or not db_trace.spans:
                    return
...
class EvalJob(Job):
    ...
    def identifier(self) -> str:
        return (
            ...
            f"run_id={self.experiment_run.id}, evaluator_name={self.name or self.evaluator.name}"
        )
    ...
    def get_rate_limit_key(self) -> Hashable:
        ...
        # (prefer job name from dataset evaluator)
        return ("builtin_evaluator", self.name or self.evaluator.name)
        ...
        try:
            with anyio.fail_after(self.timeout):
                context_dict: dict[str, Any] = {
                    ...
                    "output": self.experiment_run.output.get("task_output"),
                    "metadata": self.dataset_example_revision.metadata_,
                }
                ...
                result = eval_result[0] if eval_result else None
                if result is None:
                    logger.warning("%s: evaluator returned no result", self.identifier)
                    return
...
class RunningExperiment:
    ...
    def __init__(
        ...
        *,
        ...
        credentials: list["GenerativeCredentialInput"] | None = None,
        evaluator_by_id: dict[int, "BaseEvaluator"] | None = None,
        dataset_evaluator_name_by_id: dict[int, str] | None = None,
        max_retries: int = 3,
        ...
        self.credentials = credentials
        self.evaluator_by_id = evaluator_by_id or {}
    ...
    async def _ensure_task_buffer(self) -> None:
        ...
        prompt_version = task_config.prompt_version_config
        ...
        for revision, successful_count, incomplete_reps in rows:
            ...
            try:
                formatted_tuples = self._format_template(
                    template, template_format, template_variables
                )
                formatted_messages: list[PlaygroundMessage] = [
                    create_playground_message(role, content, tool_call_id, tool_calls)
                    for (role, content, tool_call_id, tool_calls) in formatted_tuples
                ]
    ...
    def _build_invocation_parameters(self, task_config: "TaskConfig") -> dict[str, Any]:
        ...
        Note: response_format is NOT included here — it's a separate argument
        to chat_completion_create, not part of invocation_parameters.
        """
        from phoenix.db.types.prompts import get_raw_invocation_parameters

        prompt_version = task_config.prompt_version_config
        if prompt_version.invocation_parameters is not None:
            return get_raw_invocation_parameters(prompt_version.invocation_parameters)
        return {}
    ...
    def _create_task_job(
        ...
        return TaskJob(
            ...
            invocation_parameters=invocation_parameters,
            tools=tools_list,
            prompt_version=self.config.task_config.prompt_version_config,
            db=self.db,
            decrypt=self.decrypt,
            span_cost_calculator=self.span_cost_calculator,
            project_id=project_id,
            credentials=self.credentials,
        )
    ...
    def _create_eval_job(
        ...
        """
        evaluator = self.evaluator_by_id.get(evaluator_config.dataset_evaluator_id)
        if evaluator is None:
            logger.warning(
                "Evaluator id=%s not found, skipping", evaluator_config.dataset_evaluator_id
            )
            return None

        # Prefer dataset evaluator name (user-facing) over base evaluator name
        name = (
            self._dataset_evaluator_name_by_id.get(evaluator_config.dataset_evaluator_id)
            or evaluator.name
        )
    ...
    async def on_eval_success(
        ...
        eval_chunk = EvaluationChunk(
            evaluator_name=job.name or "Evaluator",
        ...
        *,
        ...
        async with self._db() as session:
            ...
            from phoenix.server.api.types.Evaluator import (
                DatasetEvaluator as DatasetEvaluatorNode,
            )
